...
use super::ast::Stmt::*;
use super::ast::Arg::*;
use super::super::vm::bytecode::{self, Instruction, RegisterId};

#[allow(unused_imports)]
use super::super::vm::bytecode::Instruction::{Add, Sub, Mul, Div, Rem, Cmp, Neg, Lt, Le, Gt, Ge,
                                              Eq, Ne, BitNot, BitXor, BitAnd, BitOr, Lit, StParam,
                                              LdParam, Mov, JumpZero, Jump, Call};
...
impl<'a, 'b> RoutineContext<'a, 'b> {
    ...
    fn resolve(&self, name: &'a str) -> CodeGenResult<SymbolResolution> {
        return self.register_map
            .get(&name)
            .map_or_else(// No register found, try parameter
    ...
    fn compile_call(&mut self,
                    ...
                    -> CodeGenResult<()> {
        let info = try!(self.program_ctx
            .lookup_routine_info(&routine_name[..])
        ...
        if argv.len() + paramoffset < num_params {
            return Err(InsufficientArguments(routine_name.to_string(),
                                             argv.len(),
                                             num_params - paramoffset));
        }
        ...
        for arg in argv.iter() {
            debug!("call {}(#{} {:?}) should end up in register {}",
                   routine_name,
                   i,
                   arg,
                   self.eval_stack_ptr);
        ...
        for stmt in body.iter() {
            match stmt {
                ...
                &Condition(ref cond, ref ifbranch, ref elsebranch) => {
                    ...
                    if self.eval_stack_ptr == 0 {
                        return Err(InternalError("Underflow of static evaluation stack during \
                ...
                &While(ref cond, ref loop_body) => {
                    ...
                    if self.eval_stack_ptr == 0 {
                        return Err(InternalError("Underflow of static evaluation stack during \
                                                  compilation of while condition."));
                    }
...
fn infer_register_names<'a, 'd>(routine_name: &str,
                                body: &'a [Stmt],
                                parameter_names: &[&'a str],
                                program_ctx: &'a ProgramContext)
                                -> CodeGenResult<Vec<&'a str>> {
    debug!("INFER REGISTERS for routine {}({:?})",
           routine_name,
           parameter_names);
    ...
    let final_register_names: Vec<&str> = FromIterator::from_iter(repeat("<anonymous evaluation \
                                                                          stack register>")
        .take(max_stack)
    ...
    debug!(" DONE REGISTERS for routine {}({:?})\n  registers ({}):\n    {}",
           routine_name,
           parameter_names,
           final_register_names.len(),
           final_register_names.join("\n    "));
    return Ok(final_register_names);
}

fn list_variables<'a: 'p, 'p>(body: &'a [Stmt],
                              program_ctx: &'p ProgramContext<'a>)
                              -> CodeGenResult<(HashSet<&'a str>, usize)> {
    struct Ctx<'c: 'p, 'p> {
        vars: HashSet<&'c str>,
        max_stack: usize,
    ...
    fn list_vars_in_expr<'b, 'p>(ctx: &mut Ctx<'b, 'p>,
                                 expr: &'b Expr,
                                 occupied_stack: usize)
                                 -> CodeGenResult<()> {
        ...
        debug!("  end {:?} with stack size {} for a max stack size of {}",
               expr,
               size_for_expr,
               ctx.max_stack);
        Ok(())
    ...
    fn list_var_in_call<'b, 'p>(ctx: &mut Ctx<'b, 'p>,
                                routine_name: &str,
                                argv: &'b [Arg],
                                occupied_stack: usize,
                                implicit_arguments: usize)
                                -> CodeGenResult<()> {
        let num_params = try!(ctx.program_ctx
            .lookup_routine_info(routine_name)
            .map(|x| x.num_parameters)
            .ok_or_else(|| UnresolvedSymbol(routine_name.to_string())));
...
// shortcut
pub fn compile_isolated_routine<'a>(parameter_names: &'a [&str],
                                    body: &[Stmt])
    ...
    let mut routine = bytecode::Routine::new(routine_name,
                                             parameter_names.len(),
                                             register_names.len(),
                                             Vec::new());
...
impl<'p> ProgramContext<'p> {
    fn new() -> ProgramContext<'p> {
        ProgramContext { routine_map: HashMap::new() }
    }
    ...
    fn compile_routine_body<'a>(&self,
                                routine: &mut bytecode::Routine,
                                parameter_names: &'a [&str],
    ...
    for decl in decls.iter() {
        match decl {
            &Routine(ref name, ref parameter_names, ref stmts) => {
                let info = try!(ctx.routine_map
                    .get(&name[..])
                    .ok_or(InternalError("Can't find routine info that I created milliseconds \
                                          ago.")));
                ...
                let pnames: Vec<&str> = FromIterator::from_iter(parameter_names.iter()
                    .map(|x| &x[..]));

                // Prepare routine for compilation (compute registers)
                let register_names =
                    try!(infer_register_names(&name[..], &stmts[..], &pnames[..], &ctx));
                let mut routine = bytecode::Routine::new(name.clone(),
                                                         parameter_names.len(),
                                                         register_names.len(),
                                                         Vec::new());

                // Compile routine
                try!(ctx.compile_routine_body(&mut routine,
                                              &pnames[..],
                                              &register_names[..],
                                              &stmts[..]));
